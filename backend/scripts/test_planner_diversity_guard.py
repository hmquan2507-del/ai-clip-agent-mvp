...
def main() -> None:
    planner = build_ai_planner_runtime()
    ...
    raw_plan = planner.build_plan(request)
    guarded_plan = PlannerDiversityGuard().apply(raw_plan)

    print("=== Diversity Guard ===")
    print("before:", len(raw_plan.instructions))
    print("after:", len(guarded_plan.instructions))
    print("metadata:", guarded_plan.metadata)

    print("\n=== B-roll Queries ===")
    for item in guarded_plan.instructions:
        if item.instruction_type.value == "broll":
            print(
                {
                    "query": item.query,
    ...
    print("\nDONE: Planner diversity guard test completed.")
