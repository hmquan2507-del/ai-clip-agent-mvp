...
def main() -> None:
    ...
    resolved_plan = PlannerConflictResolver().resolve(plan)
    validation = PlannerValidator().validate(resolved_plan)
    graph = PlannerExecutionGraphBuilder().build(resolved_plan)
    payload = PlannerExportContract().to_payload(resolved_plan, graph)
    ...
    print("\nDONE: Planner execution contract test completed.")
