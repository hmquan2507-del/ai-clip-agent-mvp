...
def main() -> None:
    ...
    optimized_plan = PlannerOptimizer().optimize(raw_plan)

    print("=== Planner Optimizer ===")
    print("before:", len(raw_plan.instructions))
    print("after:", len(optimized_plan.instructions))
    print("metadata:", optimized_plan.metadata)

    print("\n=== Instructions ===")
    for item in optimized_plan.instructions:
        print(item.to_dict())

    print("\nDONE: Planner optimizer runtime test completed.")
