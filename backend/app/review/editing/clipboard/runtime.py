...
class TimelineClipboardRuntime:
    ...
    def state(
        ...
        return TimelineClipboardState(
            ...
            available=(
                self._content.available
            ),
            item_count=(
                self._content.item_count
            ),
            clip_count=(
                self._content.clip_count
            ),
            clipboard_id=(
                self._content.clipboard_id
            ),
            last_action=(
                self._content.action
                if self._content.available
                else None
            ),
            source_track_ids=(
                self._content
    ...
    def cut_clips(
        ...
        event_type = (
            ...
            else TimelineClipboardEventType
            .CLIPS_CUT
        )
    ...
    def paste(
        ...
        *,
        ...
        source_tracks = (
            self._content.source_track_ids
        )
        ...
        if (
            ...
            return self._failure(
                "Không thể dán nhiều source track "
                "vào một track nếu chưa cung cấp "
                "track_mapping.",
                event_type=(
                    TimelineClipboardEventType
                    .PASTE_FAILED
        ...
        for index, item in enumerate(
            ...
            clip.clip_id = (
                f"{item.source_clip_id}_"
                f"paste_{paste_group_id}_{index}"
            )

            clip.track_id = (
                resolved_mapping.get(
    ...
    def _copy_clips(
        ...
        *,
        ...
        for clip_id in normalized_ids:
            clip = current_timeline.get_clip(
                clip_id
            )
