...
const disabledCheck=!e.resolveSnap({source:source(111)}).snapped;e.removeTargets(['custom-1']);const cleared=e.clearTargets();const reset=e.reset();e.dispose();let disposed=false;try{e.resolveSnap({source:source(1)})}catch{disposed=true}
const checks={
