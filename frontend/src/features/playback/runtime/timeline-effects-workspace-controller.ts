...
export class TimelineEffectsWorkspaceController {
  ...
  getState(): TimelineEffectsWorkspaceState {
    ...
    return Object.freeze({
      ...
      presets: this.runtime.presets.list(),
  ...
  private async mutate(label: string, mergeKey: string, changes: readonly { path: string; before: TimelineHistoryJsonValue; after: TimelineHistoryJsonValue }[], operation: () => unknown | Promise<unknown>): Promise<void> {
      ...
      if (this.history) {
        await this.history.execute({
          ...
          changes: changes.map((change) => {
            ...
            const entityType = segments[0] ?? "timeline";
            const entityId = segments[1] ?? change.path;
            return Object.freeze({
              changeId: this.idFactory(),
