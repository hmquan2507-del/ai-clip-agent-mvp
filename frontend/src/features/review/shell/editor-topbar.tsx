...
import type { ReviewEditorHeaderView } from "../integration/contracts";
...
export function ReviewEditorTopbar({
  ...
  const title = view?.title ?? "Video bán hàng · Bản AI dựng";
  const productionId = view?.productionId.slice(0, 8) ?? "221e4b01";
  const duration = view?.durationLabel ?? "00:28";
  return (
    <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-[var(--review-border)] bg-[var(--review-bg-elevated)] px-3 md:px-4">
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <ReviewBadge tone="brand" className="hidden sm:inline-flex">AI Cut</ReviewBadge>
          </div>
          <p className="truncate text-[10px] text-[var(--review-text-subtle)]">
            Production {productionId} · {duration}
          </p>
        </div>
      </div>
      ...
      <div className="hidden items-center gap-2 lg:flex">
        <ReviewToolbarGroup>
          <ReviewIconButton aria-label="Hoàn tác" size="sm" disabled={!view?.canUndo}><Undo2 /></ReviewIconButton>
          <ReviewIconButton aria-label="Làm lại" size="sm" disabled={!view?.canRedo}><Redo2 /></ReviewIconButton>
        </ReviewToolbarGroup>

        <ReviewBadge tone={view?.statusTone ?? "success"} dot>
          <CloudCheck className="size-3" />
          {view?.statusLabel ?? "Đã lưu"}
        </ReviewBadge>
      </div>

      <div className="flex shrink-0 items-center gap-1.5">
        <ReviewIconButton aria-label="Xem thử video" variant="secondary" size="sm">
          <Play />
        </ReviewIconButton>
        <ReviewIconButton
