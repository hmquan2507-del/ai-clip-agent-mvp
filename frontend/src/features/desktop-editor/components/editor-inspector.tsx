...
import { BrainCircuit, Sparkles, SlidersHorizontal } from "lucide-react";

import type { ReviewAICommandBarProps } from "../../review/shell";
import { AiDecisionInspector, useAiDecisionActions } from "../../ai-decision-actions";
...
export function EditorInspector({
  ...
  const [activeTab, setActiveTab] = useState<EditorInspectorTabKey | "ai-decision">("ai-copilot");
  const decisionActions = useAiDecisionActions();
  const hasSelectedDecision = Boolean(decisionActions.selected);
    ...
    >
      <div className="flex h-9 shrink-0 items-center border-b border-[var(--desktop-editor-border-subtle)] px-1">
        ...
        <TabButton
          label="Decision"
          icon={BrainCircuit}
          active={activeTab === "ai-decision"}
          emphasized={hasSelectedDecision}
    ...
    <button
      ...
      className={
        active
          ...
          : emphasized
            ? "flex h-full flex-1 items-center justify-center gap-1.5 text-[11px] font-semibold text-[var(--desktop-editor-primary)] transition hover:text-[var(--desktop-editor-text)]"
