...
import { useAiDecisionActions } from "../../ai-decision-actions";
import type { AiDecisionAction } from "../../ai-decision-actions";
...
export function AiTimeline({ realClips = [], onBlockAction, onRequestSelectClip }: AiTimelineProps) {
  ...
  const handleSelect = useCallback(
    (block: AiBlock) => {
      ...
      decisionActions.selectDecision(block);
      if (block.linkedClipId) onRequestSelectClip?.(block.linkedClipId);
    },
    [decisionActions, onRequestSelectClip, state],
  );
  ...
  const handleContextAction = useCallback(
    (action: AiBlockAction, block: AiBlock) => {
      ...
      const actionMap: Partial<Record<AiBlockAction, AiDecisionAction>> = {
        regenerate: "regenerate",
        disable: block.disabled ? "enable" : "disable",
        duplicate: "duplicate",
        "convert-to-manual": "convert-to-manual",
        pin: block.pinned ? "unpin" : "pin",
        explain: "explain",
      };
      const decisionAction = actionMap[action];
      if (decisionAction) void decisionActions.runAction(decisionAction, block);
      onBlockAction?.({ action, block });
      state.closeContextMenu();
    },
    [decisionActions, onBlockAction, state],
  );
    ...
    >
      ...
      {state.dialog ? (
        <AiRegenerateDialog
          ...
          onRegenerate={(block) => {
            void decisionActions.runAction("regenerate", block);
